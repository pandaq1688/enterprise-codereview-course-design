# 企业 CodeReview 教学版项目 Spec

> **归档声明（2026-08-29）**  
> 本文档已归档，**不再作为产品与技术基线**。  
> 现行唯一基线：[`spec/spec.md`](../spec/spec.md)（产品名称：代码审查系统）。  
> 保留本文档仅供历史对照，实现与验收请以 `spec/spec.md` 为准。

## 1. 文档信息

- 项目目录：kdop-green-course
- 产品名称：企业 CodeReview
- 文档日期：2026-08-28
- 文档状态：设计已确认，等待书面审阅
- 技术栈：Node.js 22、ESM JavaScript、原生 Web 页面、node:test
- 原项目：kdop-green，仅作为只读参考，不允许修改

本文档是教学版的唯一总体产品与技术基线。后续编码提示词、测试、实现和验收都必须引用本 Spec，不得自行扩大范围。

## 2. 背景

现有 kdop-green 是一个持续代码评审服务，包含 Git/SVN 仓库同步、KDOP 需求关联、本机 Agent 调用、SQLite、Web 查询、通知和 Bug 回写等能力。它具备真实企业项目的复杂度，但不适合作为 SDD 和 TDD 的入门教学项目。
...
教学版保留企业项目最重要的边界、可靠性和审计能力，同时移除对学习闭环没有必要的基础设施。
...
1. 用 Spec 定义范围、接口、异常和验收标准。
2. 将复杂流程拆成职责单一且可测试的组件。
3. 对核心业务行为执行 Red、Green、Refactor。
4. 使用依赖注入隔离不可重复的 Agent 调用。
5. 区分 AI 候选判断与程序最终决策。
6. 使用结构化错误、日志、报告和状态机提升可靠性。
7. 在不改动核心流程的情况下替换 AI Provider。
8. 在不复制业务逻辑的情况下增加定时任务入口。
...
1. RemoteLlmReviewProvider：通过 API Key 调用真实远程大模型。
2. ReviewScheduler：按照本地 ReviewProfile 定时执行相同的审查用例。

这两个扩展必须复用 MVP 的 ReviewJobService、二次过滤和报告组件，不得复制主业务流程。
...
教学版采用分层单体。组件通过普通 JavaScript 对象契约和构造函数注入协作，不创建没有行为价值的抽象基类。
...
MVP 不提供页面配置这些排除目录，避免学员误删必要安全边界。
...
默认教学 checklist 从现有 C/C++ 系统清单精简，保留并发、内存边界、外部输入、错误处理、资源生命周期和控制流检查。
...
triggerType 在 MVP 固定为 MANUAL，定时扩展使用 SCHEDULED。
...
示例中的 Cursor 命令只表达参数模板，不保证与本机安装完全一致。学员必须根据本机 Cursor Agent 的帮助信息调整 command 和 args。
...
教学记录必须保存关键 Red 和 Green 命令输出摘要。禁止先实现再补一个立即通过的测试并称为 TDD。
...
增加 RemoteLlmReviewProvider，使用户可以通过真实 API Key 调用远程模型，同时保持 ReviewJobService、PromptBuilder、PostReviewPolicy 和报告模型不变。
...
MVP 扩展只支持固定分钟间隔，最小间隔为 5 分钟，不引入 Cron 表达式解析。
...
docs/
  ...
  prompts/
    使用说明
    分阶段、可复制的 vibe coding 提示词
    每阶段验收与复盘提示词
~~~

本轮文档任务不提供业务源码。学员使用提示词驱动 Cursor，按 Spec 和 TDD 逐步创建源码。
...
8. 要求展示 Red 失败原因。
9. 只实现让当前测试通过的最小代码。
10. 要求展示 Green 和完整测试结果。
11. 要求检查是否修改了范围外文件。
12. 要求汇报风险、假设和下一步，但不提前实现下一步。
...
- 让 Agent 自行修改 Spec。
- 省略失败测试证据。
- 依赖真实 Cursor 或远程 API 才能运行自动测试。
- 要求 Agent 修改 kdop-green。

## 23. 推荐教学阶段

正式逐条提示词按以下阶段编写：

1. 阅读 Spec、检查环境和建立工作约束。
2. 创建最小项目骨架和测试命令。
3. 定义领域数据结构、错误对象和任务状态机。
4. TDD 实现路径安全与请求校验。
5. TDD 实现全量源码采集。
6. TDD 实现 Git 变更采集。
7. 精简并装配固定规则与 checklist。
8. TDD 实现 PromptBuilder。
9. 定义 ReviewProvider 并实现 FakeReviewProvider。
10. TDD 实现 CursorReviewProvider。
11. TDD 实现 AI JSON 解析和 Schema 校验。
12. 按 PF-001 至 PF-010 分小步 TDD 实现二次过滤。
13. TDD 实现 JSON 报告存储。
14. TDD 实现安全 HTML 报告。
15. TDD 实现 ReviewJobService 和队列。
16. 实现 Web API 和最小页面。
17. 完成 Fake Provider 端到端验收。
18. 配置真实 Cursor 并人工验收。
19. 执行范围、安全和文档收尾检查。
20. MVP 完成后实现远程 Provider 扩展。
21. 远程 Provider 验收后实现定时任务扩展。

每个阶段可以包含多条短提示词，但一条提示词只推进一个可验证行为。
