...
- 文档路径：`spec/spec.md`
- 文档日期：2026-09-19
- 文档状态：等待书面审阅（§9 固定规则已按现场缺陷补齐；§21/§22/§23 仍为必做扩展点）
...
4. 混合 C/C++、Java、JavaScript、Python、Go、SQL/XML、Lua 输入能自动加载对应固定规则。
5. 用户只能替换或配置 review-checklist，不能从外部配置替换全局、C/C++、Java、JavaScript、Python、Go、SQL/XML、Lua 规则。
...
- Git 变更模式。
- 全量目录模式。
- C、C++、Java、JavaScript、Python、Go、SQL、XML 和 Lua 源文件。
- 固定全局、C/C++、Java、JavaScript、Python、Go、SQL/XML、Lua 规则。
...
- 输入包含 SQL 或 XML 文件时加载固定 SQL/XML 规则。
- 输入包含 Lua 文件时加载固定 Lua 规则。
- `.c` 与 C++ 扩展名都加载同一份固定 C/C++ 规则，且只加载一次。
...
4. 审查模式、文件清单和范围说明。
5. 固定全局规则。
6. 固定语言规则。有对应文件时按此顺序出现：C/C++、Java、JavaScript、Python、Go、SQL/XML、Lua。没有对应文件则不出现该段。
...
固定规则是交给 Agent 的 Markdown 检查动作，不是另一套工具或输出协议。规则只收已经出现过、换项目仍然成立的失败模式。产品名、固定目录、专属宏、功能号和数据库供应商方言不得写入固定规则；它们只能出现在显式 checklist。

以下内容明确不进入固定规则：

- 空的产品 checklist 模板。默认 checklist 仍只保留并发、内存边界、外部输入、错误处理、资源生命周期和控制流。
- 要求运行某一款静态分析命令。C/C++、Python、Go 的外部工具分别是 clang-tidy、ruff、go vet（§22）。工具告警只是候选，必须经代码确认，不得伪造工具结果。
- KDOP、仓库轮询、提交作者、邮件、企微、工单和固定版本库地址。
- 新的“待核实”输出字段。证据不足时不得维持 MEDIUM、HIGH 或 CRITICAL，按 §10 和 §11 处理。

### 9.1 固定全局规则

全局规则必须覆盖：

- 只基于需求、源码和 diff 中的证据。正式问题必须有文件、起止行、代码证据、根因、影响和只读修复建议。
- 不猜测外部接口、未知运行环境、未提供的依赖或部署细节。
- 优先发现正确性、安全性、内存安全、并发和资源生命周期问题。
- `return`、`break`、`continue`、`goto` 不得跳过清理、状态推进、计数或错误传播。成功、部分成功、EOF、重试、取消和异常路径仍须满足调用契约。
- 注入、未授权、越权、路径穿越、不安全反序列化、敏感日志和硬编码凭据，仅在入口、数据流和影响都能核对时报告。
- 追踪 fd、socket、锁、线程、连接、缓存、回调和第三方句柄的获取与释放。检查泄漏、重复释放、悬空引用、use-after-free，以及只增不删或没有容量上限的缓存。
- 数据竞争、非线程安全集合、锁顺序、死锁、锁内阻塞、线程膨胀、迟到回调和回调移除时仍在执行。队列、重连、重试、日志和任务必须有容量、退避、超时或停止条件。
- 多对象更新的原子性、回滚或补偿、状态机不变量，以及生产者/消费者、Schema、代码和接口字段一致性。
- 金额、数量、费率、时间和单位的类型、精度、边界和舍入。具体业务口径只来自需求或 checklist。
- 性能问题必须有规模、调用频率或明确路径证据。可维护性问题只报告有实际影响的重复、逻辑冲突、多余逻辑、职责混杂、耦合、硬编码和死代码。
- 明确需求与代码冲突时允许报告需求不符合，且必须同时给出需求引用和代码位置。
- 无准确位置和证据的中高风险结论不成立。同一根因只报告一次。
- 输出必须符合 JSON Schema。除代码、符号和路径外，报告文本使用简体中文。

### 9.2 固定 C/C++ 规则

`.c` 与 C++ 扩展名加载同一份规则，且只加载一次。规则必须写成可核对的检查动作，并覆盖：

- 未初始化使用和未定义行为；空指针、悬空指针、use-after-free、重复释放；数组和缓冲区越界。
- 原子与内存序：`volatile` 不是同步手段。先写数据再设完成标志时，写端 release 与读端 acquire 必须配对。relaxed 不发布旁边的普通数据。多原子字段不自动构成一致快照。
- 对象生命周期：锁必须覆盖查找、固定生命周期和使用。只锁查找后返回裸指针不够。超时、重连或停止后的迟到回调不得访问已释放或已复用对象。回调移除与正在执行的回调之间必须有 drain、join、引用计数或等价同步。提前返回不得造成手工加锁永不解锁。
- 等待与死锁：自旋必须有暂停或退避、超时和停止条件。在非原子共享变量上等待仍是数据竞争。检查锁顺序、回调重入和锁内阻塞调用。
- 共享内存：attach 前核对 magic、版本、长度和初始化完成标志。不得用新配置重算已有共享内存内部指针。映射长度不得大于文件实际长度。仍有附着者时不得重建共享环。持锁进程死亡不得让锁永久占住。恢复路径不得依赖可能已损坏的共享数据。
- 信号与进程：信号处理链上不得调用分配、加锁、日志或其他非 async-signal-safe 操作。`fork` 返回值按 `pid_t` 分三条路径处理，不得先存入无符号字段。`kill` 前确认 pid 大于 0。`exec` 失败使用 `_exit`。`waitpid` 先判断退出方式再读状态。停止顺序为 stop、join 或 drain、close、delete，且重复调用幂等。
- 外部输入与边界：先验证头长度再验证 body。用剩余长度比较，不得先计算可能回绕的 `head + body`。`size_t` 减法前确认被减数不小于减数。拷贝不超过目标容量并保证 NUL 终止。`snprintf` 返回值是所需长度，不得直接当作已写长度。索引在最终数组处再验证。分配与释放成对；扩容失败后旧指针仍可用。回调不得保存栈对象或会失效的容器元素地址。
- 协议与状态机：分页加压缩、分片加队列满、订阅加重连、超时加迟到应答要分开看。循环的每个分支都推进游标或显式退出。解压后的缓冲区不得与原始缓冲区交叉使用。超时取消后不得把迟到应答写入已释放槽位。
- 错误处理：区分错误、部分成功、EOF、EINTR、EAGAIN 和超时。短读短写必须处理。失败路径回滚已更新的计数、注册和状态。返回码类型必须能表达负错误值。不得吞掉队列满、上报失败或部分发送。
- 长稳：按获取与释放核对 fd、映射、锁、线程、socket 和第三方句柄，覆盖失败出口、重试、重连和析构。缓存和回调表不得只增不删。超时间隔使用单调时钟。
- JNI 与 CPython：数组元素获取与释放成对；常驻线程 Attach/Detach；跨回调保存的 Java 对象使用 global reference；异常挂起时不得继续调用大多数 JNI API。Python 侧分清引用所有权，非持有运行时的线程访问前获取 GIL，不得在留下异常时返回成功。
- 整数与可移植性：乘法前提升到目标宽度。有符号与无符号转换、负错误码进入无符号、除零和格式化宽度不匹配都要检查。字符串比较比较内容而不是指针。ARM 上不得依赖 x86 强内存序。

删除企业私有宏、固定目录、数据库方言和项目专属命名。规则文本不要求运行 cppcheck 或其他特定静态分析命令。

### 9.3 固定 Java 规则

规则必须写成可核对的检查动作，并覆盖：

- 空指针和 Optional 误用；资源在所有路径关闭；禁止吞异常后继续处于错误状态。
- `ThreadLocal` 在 finally、拦截器或请求结束路径清理。线程池复用线程时，未清理的值会串到后续任务。
- 遍历时修改集合，以及非线程安全集合的共享访问。
- 静态集合和缓存不得只增不减，不得无上限加载大结果集。
- 线程池使用有界队列和明确拒绝策略，并处理停止、取消和任务异常。禁止无界线程工厂或无界队列。
- 事务边界。同类自调用导致声明式事务失效时，必须能指出调用路径。
- 外部入参的空值、长度、范围和格式。
- 金额不得用 `double` 直接计算，也不得 `new BigDecimal(double)`。检查 scale、舍入、溢出和输入边界。
- 强制类型转换必须有类型证据。
- 远程调用必须能看到连接超时和读取超时。重试有限。缺少幂等只在输入能证明应当幂等时报告。
- 魔法数字和调试残留只在改变金额、状态、权限或协议语义时报告。

删除固定数据库方言、固定目录和企业内部约定。
...
`.sql` 与 `.xml` 加载同一份规则。只报告输入里能直接核对的问题。必须覆盖：

- 常规参数使用绑定参数，例如 MyBatis `#{ }`。`${ }`、拼接 SQL、动态表名或排序字段必须有白名单和可信来源，否则按注入报告。
- `SELECT` 显式列出字段。`INSERT` 显式列出目标列。
- `DELETE` 和 `UPDATE` 必须有可验证范围。能定位的无条件全表更新或删除按高影响数据错误报告。
- 字段类型、默认值、非空、返回映射，以及 Schema、XML 和代码字段一致。
- XML 能解析：标签闭合、参数名与代码属性一致、结果映射没有明显错列。
- 分页和事务只在语句或调用处有证据时检查。
- 性能结论需要查询规模、索引或调用频率证据。

不把数据库供应商方言、备份命令、固定目录或功能号例外写入固定规则。未知方言行为不报中高风险。

### 9.8 固定 Lua 规则

`.lua` 加载本规则。未提供的运行时、宿主框架或业务 API 不猜测。必须覆盖：

- 区分 `nil` 与 `false`。检查未声明全局变量、table 键生命周期、metatable、闭包捕获，以及错误返回是否被调用方处理。
- 协程 resume 与 yield 成对。回调取消后不得访问已释放状态。循环有终止条件。字符串和二进制长度不越界。事件或回调不得只注册不注销。
- 与 C/C++ 交互时，每条路径上 Lua 栈平衡；userdata 生命周期覆盖使用点；错误对象被传播或明确清理；整数与浮点转换不静默截断；非持有运行时的线程不得直接访问 Lua 状态。

### 9.9 可配置 review-checklist
...
全局、C/C++、Java、JavaScript、Python、Go、SQL/XML 和 Lua 规则路径不出现在外部配置中，由 RuleResolver 固定引用。这里的“只有 checklist 可配置”只限制审查规则；端口、允许根目录、超时和输入上限等运行参数仍可配置。
...
- ruleId。
- ruleType：GLOBAL、CPP、JAVA、JS、PYTHON、GO、SQL_XML、LUA 或 CHECKLIST。
- builtIn。
- contentHash。
- matchPaths。
- matchedFiles。
...
- 外部配置无法替换全局、C/C++、Java、JavaScript、Python、Go、SQL/XML 或 Lua 规则。
...
- 同时存在 `.c` 与 C++ 文件时，只加载一份 C/C++ 规则，匹配文件包含两者。
- `.sql` 与 `.xml` 加载同一份 SQL/XML 规则。`.lua` 加载 Lua 规则。没有这些文件时不加载对应规则。
- 全局规则包含同一根因只报告一次。C/C++ 规则包含共享内存检查。Java 规则包含 `ThreadLocal` 与 `BigDecimal(double)`。SQL/XML 规则包含绑定参数。Lua 规则包含 `nil` 与 `false`。
- 固定规则正文不要求运行 cppcheck，也不包含产品名、固定目录、专属宏或数据库供应商方言。
